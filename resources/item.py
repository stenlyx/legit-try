...
class Brand(Resource):
    ...
    def get(cls, brand: str):
        return {"items": item_list_schema.dump(ItemModel.find_all_by_brand(brand))}


class New(Resource):
    @classmethod
    def get(cls):
        return {"items": item_list_schema.dump(ItemModel.find_if_new())}


class Bestseller(Resource):
    @classmethod
    def get(cls):
        return {"items": item_list_schema.dump(ItemModel.find_if_bestseller())}


class Sale(Resource):
    @classmethod
    def get(cls):
        return {"items": item_list_schema.dump(ItemModel.on_sale())}
